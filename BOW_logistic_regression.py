...
def transform_data(dataset,unique_words):
    processed_data_set = pd.DataFrame(columns = unique_words)
    for idx,row in dataset.iterrows():
        data = [0]*len(unique_words)
        for i in range(len(unique_words)):
...
def apply_bow_logistic_regression_model(file_path):
    class_label = ['0','1']
    ...
    X_validation = transform_data(X_validation,train_unique_words)
    ...
    X_total_train_data = transform_data(X_total_train_data,total_train_unique_words)
...
if __name__ == '__main__':
    ...
    path =str(arg_list[1])
    accuracy, precision, recall,F1 = apply_bow_logistic_regression_model(path)
    print("Metrics in Logistic Regression-Bag Of Words")
